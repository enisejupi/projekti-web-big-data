...
# Ekzekuton tÃ« gjithÃ« procesin: test, deploy, start cluster, start app
...
if (-not $SkipTest) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "`nâŒ Connection test failed. Please check VMs." -ForegroundColor Red
        exit 1
    }
...
if (-not $SkipDeploy) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "`nâŒ Deployment failed." -ForegroundColor Red
        exit 1
    }
...
if (-not $SkipCluster) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "`nâŒ Cluster start failed." -ForegroundColor Red
        exit 1
    }
    
    Write-Host "`nâœ“ Cluster started!" -ForegroundColor Green
    Start-Sleep -Seconds 3
} else {
